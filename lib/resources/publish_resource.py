...
def _aggregate_ird(moh_response):
    daily_ird_dict = {
        current_day['date']: {'infected': current_day['amount'], 'recovered': current_day['recovered'],
                              'date': current_day['date']} for current_day in moh_response[1]['data'][-20:]}

    for current_day in moh_response[5]['data'][-20:]:
...
def _aggregate_test_amount(moh_response):
    ...
    for current_day in moh_response[6]['data'][-20:]:
        test_amount.append({
...
def _aggregate_vaccinations(moh_response):
    return [
        {
        ...
        } for current_day in moh_response[7]['data'][-20:]
    ]
...
class PublishResource(Resource):
    ...
    def post(self):
        ...
        daily_vaccinations_data = self.gen_post_struct('dailyVaccinationsData', _aggregate_vaccinations(moh_response))
